...
inline void FixBackbonepH::print_log(char *line)
{
  if (screen) fprintf(screen, line);
  if (logfile) fprintf(logfile, line);
}

FixBackbonepH::FixBackbonepH(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
void FixBackbonepH::final_log_output()
{
  double time, tmp;
...
//-----------------------------------------------------
  ...
  if (water_flag) {
    ...
    well = new cWell<double, FixBackbonepH>(n, n, n_wells, water_par, &ntimestep, this);
  }
  ...
  if (helix_flag) {
    ...
    helix_well = new cWell<double, FixBackbonepH>(n, n, n_helix_wells, helix_par, &ntimestep, this);
  }

  if (p_ap_flag) {
    p_ap = new cP_AP<double, FixBackbonepH>(n, n, &ntimestep, this);
  }

  R = new cR<double, FixBackbonepH>(n, n, &ntimestep, this);
...
/* ---------------------------------------------------------------------- */
inline bool FixBackbonepH::isFirst(int index)
{
...
inline bool FixBackbonepH::isLast(int index)
{
...
int FixBackbonepH::Tag(int index) {
  if (index==-1) return -1;
  return atom->tag[index];
}

inline void FixBackbonepH::Construct_Computational_Arrays()
{
  int *mask = atom->mask;
...
void FixBackbonepH::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixBackbonepH::setup(int vflag)
{
...
void FixBackbonepH::min_setup(int vflag)
{
  pre_force(vflag);
}
...
inline double FixBackbonepH::PeriodicityCorrection(double d, int i)
{
  if (!periodicity[i]) return d;
  else return ( d > half_prd[i] ? d - prd[i] : (d < -half_prd[i] ? d + prd[i] : d) );
}

bool FixBackbonepH::isEmptyString(char *str)
{
  int len = strlen(str);
...
char *FixBackbonepH::ltrim(char *s) 
{     
  while(isspace(*s)) s++;     
  return s; 
}  

char *FixBackbonepH::rtrim(char *s) 
{
  char* back;
  int len = strlen(s);
...
char *FixBackbonepH::trim(char *s) 
{     
  return rtrim(ltrim(s));  
} 

Fragment_Memory **FixBackbonepH::read_mems(char *mems_file, int &n_mems)
{
  int file_state, nstr;
  int tpos, fpos, len;
  double weight;
...
void FixBackbonepH::timerEnd(int which)
{
  // uncomment if want synchronized timing
  // MPI_Barrier(world);
...
void FixBackbonepH::compute_chain_potential(int i)
{	
...
void FixBackbonepH::compute_shake(int i)
{	
  double dx[3], r, dr, force;
...
void FixBackbonepH::compute_chi_potential(int i)
{
  double dx[3], r, dr, force;
...
void FixBackbonepH::calcDihedralAndSlopes(int i, double& angle, int iAng)
{
  double a[3], b[3], c[3];
...
void FixBackbonepH::compute_rama_potential(int i)
{
...
void FixBackbonepH::compute_excluded_volume()
{ 
  double dx[3], r, dr, force;
...
void FixBackbonepH::compute_p_degree_excluded_volume()
{ 
  int sign = (p%2==0 ? 1 : -1);
...
void FixBackbonepH::compute_r6_excluded_volume()
{
...
inline double FixBackbonepH::anti_HB(int res1, int res2, int k)
{
  return m_anti_HB[se_map[res1-'A']][se_map[res2-'A']][k];
}

inline double FixBackbonepH::anti_NHB(int res1, int res2, int k)
{
  return m_anti_NHB[se_map[res1-'A']][se_map[res2-'A']][k];
}

inline double FixBackbonepH::para_HB(int res1, int res2, int k)
{
  return m_para_HB[se_map[res1-'A']][se_map[res2-'A']][k];
}

inline double FixBackbonepH::para_one(int res)
{
  return m_para_one[se_map[res-'A']];
}

inline double FixBackbonepH::anti_one(int res)
{
  return m_anti_one[se_map[res-'A']];
}

inline double FixBackbonepH::get_water_gamma(int i_resno, int j_resno, int i_well, int ires_type, int jres_type, int water_prot_flag)
{
...
inline double FixBackbonepH::get_burial_gamma(int i_resno, int ires_type, int local_dens)
{
  if (!phosph_flag) {
...
void FixBackbonepH::compute_dssp_hdrgn(int i, int j)
{
...
void FixBackbonepH::compute_P_AP_potential(int i, int j)
{
...
void FixBackbonepH::compute_water_potential(int i, int j)
{	
...
void FixBackbonepH::compute_burial_potential(int i)
{
...
void FixBackbonepH::compute_helix_potential(int i, int j)
{
...
void FixBackbonepH::compute_amhgo_normalization()
{
...
void FixBackbonepH::compute_amh_go_model()
{
...
void FixBackbonepH::compute_vector_fragment_memory_potential(int i)
{
  int j, js, je, i_fm;
...
/*void FixBackbonepH::compute_vector_fragment_memory_table()
{
...
void FixBackbonepH::compute_fragment_memory_potential(int i)
{
...
void FixBackbonepH::table_fragment_memory(int i, int j)
{
...
void FixBackbonepH::compute_decoy_memory_potential(int i, int decoy_calc)
{
...
void FixBackbonepH::randomize_decoys()
{
...
// is necessary.
void FixBackbonepH::compute_fragment_frustration()
{
...
void FixBackbonepH::compute_generated_decoy_energies()
{
...
void FixBackbonepH::output_selection_temperature_data()
{
  double *xi, *xj, dx[3];
  int i, j;
...
void FixBackbonepH::compute_mcso()
{
  int i;
...
double FixBackbonepH::compute_total_burial_energy()
{
  int i;
...
double FixBackbonepH::compute_total_contact_energy()
{
  int i, j;
  int ires_type, jres_type, i_resno, j_resno, i_chno, j_chno;
  double rij, rho_i, rho_j;
...
void FixBackbonepH::compute_tert_frust()
{
  double *xi, *xj, dx[3];
  int i, j;
  int ires_type, jres_type, i_resno, j_resno, i_chno, j_chno;
...
void FixBackbonepH::compute_tert_frust_singleresidue()
{
  int i;
  int ires_type, i_resno, i_chno;
  double rho_i;
...
double FixBackbonepH::compute_native_ixn(double rij, int i_resno, int j_resno, int ires_type, int jres_type, double rho_i, double rho_j)
{
...
void FixBackbonepH::compute_decoy_ixns(int i_resno, int j_resno, double rij_orig, double rho_i_orig, double rho_j_orig)
{
...
double FixBackbonepH::compute_singleresidue_native_ixn(int i_resno, int ires_type, double rho_i, int i_chno, double cutoff, bool nmercalc)
{
...
void FixBackbonepH::compute_singleresidue_decoy_ixns(int i_resno, double rho_i, int i_chno)
{
...
double FixBackbonepH::compute_array_mean(double *array, int arraysize)
{
  double mean;
  int i;
...
double FixBackbonepH::compute_array_std(double *array, int arraysize)
{
  double mean, std;
  int i;
...
double FixBackbonepH::compute_water_energy(double rij, int i_resno, int j_resno, int ires_type, int jres_type, double rho_i, double rho_j)
{
...
double FixBackbonepH::compute_burial_energy(int i_resno, int ires_type, double rho_i)
{
  double t[3][2];
...
double FixBackbonepH::compute_electrostatic_energy(double rij, int i_resno, int j_resno, int ires_type, int jres_type)
{
...
int FixBackbonepH::get_random_residue_index()
{
  int index;
  index = rand() % n; 
  return index;
...
double FixBackbonepH::get_residue_distance(int i_resno, int j_resno)
{
  double dx[3];
  double *xi, *xj;
  double r;
...
double FixBackbonepH::get_residue_density(int i)
{
  return well->ro(i);
}

// returns the residue type of residue i_resno
int FixBackbonepH::get_residue_type(int i_resno)
{
...
double FixBackbonepH::compute_frustration_index(double native_energy, double *decoy_stats)
{
  double frustration_index;
...
void FixBackbonepH::compute_nmer_frust()
{
...
void FixBackbonepH::compute_singlenmer_frust()
{
  double native_energy, frustration_index;
...
double FixBackbonepH::compute_singlenmer_native_ixn(int i_resno)
{
  int j, j_resno, jres_type, j_chno;
...
void FixBackbonepH::compute_singlenmer_decoy_ixns(int i_resno)
{
  double rij, rho_i, rho_j;
...
int FixBackbonepH::get_nmer_ss_dist(char *nmer_ss_j, char *nmer_ss_k)
{
  int i;
  int ss_dist;
...
int FixBackbonepH::compute_nmer_traps(int i_start, int j_start, int atomselect, double threshold_energy, char *nmer_seq_1, char *nmer_seq_2)
{
  int k_start;
...
int FixBackbonepH::compute_nmer_contacts(int i_start, int j_start)
{
  int numcontacts;
  int i,j;
  double distance;
...
void FixBackbonepH::get_nmer_seq(int i_start, char *nmer_seq, int backward)
{
  int i;
...
void FixBackbonepH::get_nmer_secondary_structure(int i_start, char *nmer_secondary_structure)
{
  int i;
...
double FixBackbonepH::compute_nmer_native_ixn(int i_start, int j_start)
{
...
void FixBackbonepH::compute_nmer_decoy_ixns(int i_start, int j_start)
{
...
void FixBackbonepH::compute_fragment_memory_table()
{
  int i, j, js, je, ir, i_fm, k, itb, iatom[4], jatom[4], iatom_type[4], jatom_type[4];
  int i_first_res, i_last_res, i_resno, j_resno, ires_type, jres_type;
  double r, rf, dr, drsq, V, force;
...
void FixBackbonepH::output_fragment_memory_table()
{
...
void FixBackbonepH::compute_membrane_potential(int i)
{
...
void FixBackbonepH::compute_solvent_barrier(int i, int j)
{
...
//----- AGREGADO FRANCO ------
void FixBackbonepH::mc_charge_change(double *charge_on_residues, int total_res_charged, int *charged_indexes, double *pKas, char *letter)
{
/*	void FixBackbonepH::mc_charge_change(double *charge_on_residues, int total_res_charged,
...
/*
	for(int i=0;i<numTotalCargadas;i++){
     if (i != cambio)
     {
       energia_vieja+=electro_i_j(cargadas[cambio], cargadas[i], array);
     }
    }
...
double FixBackbonepH::temp_calc(int igroup){
    char **arg = new char*[3];
    arg[0] = (char *) "velocity_temp";
    arg[1] = group->names[igroup]; // QUE SIGNIFICA ACA IGROUP VER CON MAS CUIDADO
    arg[2] = (char *) "temp";
...
int FixBackbonepH::charge_flip(int charge, char letter){
	int charge_flipped;
...
double FixBackbonepH::electro_i_j(int i, int j, double *array){ 


//Calculo de la energia electrostatica entre i y j

	  if (abs(i-j)<debye_huckel_min_sep) return 0.0; //Distancia en residuos minima necesaria
	  ...
	  charge_i = array[i];  // Cambie charge on residue por array que es lo que entra a la funcion
	  charge_j = array[j]; 
	  
	  if (charge_i == 0 && charge_j == 0) return 0.0; // Esto despues no va a hacer falta
												  // Voy a hacer la cuenta solo con los cargados directamente
	  ...
	  if (se[i_resno]=='G') { xi = xca[i]; iatom = alpha_carbons[i]; } // Si es una G la posicion viene dada por el alpha
	  else { xi = xcb[i]; iatom  = beta_atoms[i]; }                    // Si no viene dada por el beta. Para el i y para el j
	  if (se[j_resno]=='G') { xj = xca[j]; jatom = alpha_carbons[j]; }
	  else { xj = xcb[j]; jatom  = beta_atoms[j]; }
	  
	  dx[0] = xi[0] - xj[0];     // Diferencias entre posiciones
	  dx[1] = xi[1] - xj[1];
	  dx[2] = xi[2] - xj[2];
	  
	  r = sqrt(dx[0]*dx[0]+dx[1]*dx[1]+dx[2]*dx[2]);   // Delta r entre residuos
	  
	  if( (charge_i > 0.0) && (charge_j > 0.0) ) {
	    term_qq_by_r = k_PlusPlus*charge_i*charge_j/r;
	  }
	  else if(charge_i < 0.0 && charge_j < 0.0) {
	    term_qq_by_r = k_MinusMinus*charge_i*charge_j/r;        // Valores de cargas y k electro segun los casos
	  }
	  else if( (charge_i < 0.0 && charge_j > 0.0) || (charge_i > 0.0 && charge_j < 0.0)) {
	    term_qq_by_r = k_PlusMinus*charge_i*charge_j/r ;           // ¿No esta dividido por r?
	  }
	  
	  double term_energy = epsilon*term_qq_by_r*exp(-k_screening*r/screening_length);  // Valor de la energia

	  return term_energy;
	  

	  /*----------------NO NECESITO SUMAR A LA ENERGIA TOTAL--------------------------------------------
	  energy[ET_DH] += term_energy; // Supongo que esto suma el recien calculado a la energia total
	  -------------------------------------------------------------------------------------------------*/


	  /*--------------NO  NECESITO LA PARTE DE LA FUERZA-------------------------------------------------
	  force_term = (term_energy/r)*(1.0/r + k_screening/screening_length); // Calculo de la fuerza
	  
	  f[iatom][0] += force_term*dx[0];
	  f[iatom][1] += force_term*dx[1];
	  f[iatom][2] += force_term*dx[2];
	  									// Suma la fuerza a la total
...
void FixBackbonepH::compute_DebyeHuckel_Interaction(int i, int j)
{
  ...
  {
    //fprintf(screen, "residuei =%5d, charge on residuei =%8.6f\n", i, charge_i);
    //fprintf(screen, "residuej =%5d, charge on residuej =%8.6f\n", j, charge_j);
  }
  //------- AGREGADO FRANCO FIN -------- */
...
void FixBackbonepH::compute_debyehuckel_optimization()
{
...
void FixBackbonepH::read_amylometer_sequences(char *amylometer_sequence_file, int amylometer_nmer_size, int amylometer_mode)
{
...
void FixBackbonepH::compute_amylometer()
{
...
void FixBackbonepH::compute_optimization()
{
...
void FixBackbonepH::compute_average_sequence_optimization()
{
  // computes and writes out the energies for all interaction types 
  // for direct, protein-mediated, and water-mediated potentials
...
double FixBackbonepH::compute_direct_energy(double rij, int i_resno, int j_resno, int ires_type, int jres_type, double rho_i, double rho_j)
{
...
double FixBackbonepH::compute_proteinmed_energy(double rij, int i_resno, int j_resno, int ires_type, int jres_type, double rho_i, double rho_j)
{
...
double FixBackbonepH::compute_watermed_energy(double rij, int i_resno, int j_resno, int ires_type, int jres_type, double rho_i, double rho_j)
{
...
void FixBackbonepH::compute_burial_optimization()
{
  // computes and writes out the energies for all interaction types 
  // for direct, protein-mediated, and water-mediated potentials
...
// Mutate the sequence
void FixBackbonepH::mutate_sequence()
{
...
void FixBackbonepH::output_per_residue_contacts()
{
  int i, j;
  int ires_type, jres_type, i_resno, j_resno, i_chno, j_chno;
  double rij;
...
void FixBackbonepH::print_forces(int coord)
{
  int index;
...
void FixBackbonepH::compute_backbone()
{     
...
void FixBackbonepH::pre_force(int vflag)
{
  if (amylometer_flag) {
    compute_amylometer();
  }
...
void FixBackbonepH::pre_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixBackbonepH::min_pre_force(int vflag)
{
  pre_force(vflag);
}
...
double FixBackbonepH::compute_scalar()
{
...
double FixBackbonepH::compute_vector(int nv)
{
  // only sum across procs one time
