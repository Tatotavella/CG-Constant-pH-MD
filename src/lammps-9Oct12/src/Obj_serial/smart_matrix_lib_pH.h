...
class cP_AP_pH {
public:
	cP_AP_pH(int n, int m, int *indicator, U *lclass);
	~cP_AP_pH();
...
template <typename T, typename U>
cP_AP_pH<T, U>::cP_AP_pH(int nn, int mm, int *indicator, U *lclass)
{
	n = nn;
	m = mm;
	ind = indicator;
...
template <typename T, typename U>
inline T &cP_AP_pH<T, U>::nu(int i, int j)
{
...
template <typename T, typename U>
inline T &cP_AP_pH<T, U>::prd_nu(int i, int j)
{
	if ( (ind && g[i][j]!=*ind) || (!ind && g[i][j]!=1) ) {
		compute(i, j);
		if (ind) g[i][j] = *ind;
...
template <typename T, typename U>
void cP_AP_pH<T, U>::compute(int i, int j)
{
	T dx[3], dr, th;
...
class cR_pH {
public:
	cR_pH(int n, int m, int *indicator, U *lclass);
	~cR_pH();
...
template <typename T, typename U>
cR_pH<T, U>::cR_pH(int nn, int mm, int *indicator, U *lclass)
{
	n = nn;
	m = mm;
	ind = indicator;
...
template <typename T, typename U>
inline T &cR_pH<T, U>::rNO(int i, int j)
{
...
template <typename T, typename U>
inline T &cR_pH<T, U>::rHO(int i, int j)
{
...
class cWell_pH {
public:
	cWell_pH(int n, int m, int nw, const WPV &par, int *indicator, U *lclass);
	~cWell_pH();
...
template <typename T, typename U>
cWell_pH<T, U>::cWell_pH(int nn, int mm, int ww, const WPV &p, int *indicator, U *lclass)
{
	n = nn;
	m = mm;
	nw = ww;
...
template <typename T, typename U>
inline T &cWell_pH<T, U>::theta(int i, int j, int i_well)
{
...
template <typename T, typename U>
inline T &cWell_pH<T, U>::prd_theta(int i, int j, int i_well)
{
	if ( (ind && gTheta[i_well][i][j]!=*ind) || (!ind && gTheta[i_well][i][j]!=1) ) {
		compute_theta(i, j, i_well);
		if (ind) gTheta[i_well][i][j] = gTheta[i_well][j][i] = *ind;
...
template <typename T, typename U>
inline T &cWell_pH<T, U>::sigma(int i, int j)
{
...
template <typename T, typename U>
inline T &cWell_pH<T, U>::prd_H(int i)
{
	if ( (ind && gH[i]!=*ind) || (!ind && gH[i]!=1) ) {
		compute_H(i);
		if (ind) gH[i] = *ind;
...
template <typename T, typename U>
inline T &cWell_pH<T, U>::ro(int i)
{
...
template <typename T, typename U>
void cWell_pH<T, U>::compute_theta(int i, int j, int i_well)
{
...
template <typename T, typename U>
void cWell_pH<T, U>::compute_sigma(int i, int j)
{	
	v_sigma[i][j] = v_sigma[j][i] = H(i)*H(j);
}

template <typename T, typename U>
void cWell_pH<T, U>::compute_H(int i)
{	
	T g, th;
...
template <typename T, typename U>
void cWell_pH<T, U>::compute_ro(int i)
{
  int j;
