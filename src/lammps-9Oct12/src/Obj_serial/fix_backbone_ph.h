...
FixStyle(backbone_ph,FixBackbonepH)

#else

#ifndef LMP_FIX_BACKBONEPH_H
#define LMP_FIX_BACKBONEPH_H
...
class FixBackbonepH : public Fix {
 public:
  // Standart lammaps interface
  FixBackbonepH(class LAMMPS *, int, char **);
  ~FixBackbonepH();
  int setmask();
  void init();
  void setup(int);
...
//  TBV ****fm_table;
 ...
 // double pKa1;
 // double pKa2;
  void mc_charge_change(double *charge_on_residues, int total_res_charged, int *charged_indexes, double *pKas, char *letter);
  double electro_i_j(int i, int j, double *array);
  double temp_calc(int igroup);
  int charge_flip(int charge, char letter);
  //---- AGREGADO FRANCO ------
  ...
  cP_AP<double, FixBackbonepH> *p_ap;
  cR<double, FixBackbonepH> *R;
  cWell<double, FixBackbonepH> *well;
  cWell<double, FixBackbonepH> *helix_well;
