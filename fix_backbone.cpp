...
//------------FRANCO-----------------------
  ...
  int chrg_idx;
	for(i = 0;i<total_res_charged;i++){
		input_mc >> chrg_idx >> letter[i] >> init_chrg;
    charged_indexes[i] = chrg_idx;
    charge_on_residue[chrg_idx] = init_chrg;
    for(j = 0; j<len; j++){
  ...
  fprintf(screen,"ResNum\tResType\tpKa\tInitCharge\n");
	...
	for(i = 0;i<total_res_charged;i++){
		fprintf(screen,"%d\t%c\t%.2f\t%.2f\n",charged_indexes[i],letter[i],pKas[i],charge_on_residue[charged_indexes[i]]);
	}
	fprintf(screen, "Monte Carlo frequency = %d \n", freqMC );
  fprintf(screen, "Data output frequency = %d \n", freqOUT );
  fprintf(screen, "Simulation pH = %.2f \n", pH );
  fprintf(screen, "Simulation Temperature = %.2f \n", temp_montecarlo );
	fprintf(screen, "\n--------------------------------\n");
  ...
  input_wrsh >> alpha_nonpol;
  input_wrsh >> rpol;
  input_wrsh >> rnonpol;
  input_wrsh >> alpha_u_pol;
  input_wrsh >> alpha_u_nonpol;
  input_wrsh >> NpolMax;
  input_wrsh >> NnonpolMax;
  //A_self parameters
  ...
  for(i = 0;i<total_res_charged;i++){
    ...
    //Now j is the correct one
    A_selfpol_vec[i] = data_pol[j];
...
//Allocation
  ...
  int rsd,Nu_pol,Nu_nonpol;
  double D_ele,D_pH,D_self, V_electro;
...
double FixBackbone::temp_calc(int igroup){
    ...
    arg[1] = group->names[igroup]; // Careful with igroup meaning
    arg[2] = (char *) "temp";
...
int FixBackbone::charge_flip(int charge, char letter){
	int charge_flipped;
	if(letter=='D' || letter=='E'|| letter=='C' || letter=='B' || letter=='Y'){ // Acid
		if(charge==-1){
			charge_flipped=0;
		  }else if(charge==0){
			charge_flipped=-1;
...
//Electrostatic energy calculation if neither i nor j are terminals
...
//Electrostatic energy calculation if i or j are terminals
	...
	charge_i = array[i];  // charge_on_residue has 2 extra spaces
	charge_j = array[j];
...
double FixBackbone::delta_electrostatics(int rnd_idx, int place_change){
  ...
  // Compute electrostatic interactions of the residue in place_change
...
double FixBackbone::delta_ph(int rnd_idx, int place_change, double mc_temp, int old_chrg, int new_chrg){
  //Returns Delta pH of residue in place_change
...
double FixBackbone::delta_self(int rnd_idx, int place_change, int new_chrg, double *Nu_pol, double *Nu_nonpol){
  //Returns Delta Self for residue in place_change
	double Npol = 0.0;
  double Nnonpol = 0.0;
	int resno,res_type;
	char let;
