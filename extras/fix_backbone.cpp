    ...
    for(int ires = 0; ires<total_residues; ires++)
      {
		...
		//------------FRANCO--------------------------
		//indices_cargados[ires] = res_min_one;
		//--------------------------------------------
      }
    input_charge.close();
...
FixBackbone::~FixBackbone()
{
  ...
  if (huckel_flag) {
    ...
    delete[]letter;
    delete[]times_uncharged;

	//delete[]indices_cargados;
//--------------------FRANCO--------------------------------
/*	
	delete freqMC;
	delete pH;
...
//-----------------FRANCO-----------------------------
	...
	//indices_cargados=new int[total_res_charged];
	//for (i = 0; i < total_res_charged; ++i) {
	//	indices_cargados[i]=0;
	//}
...
//--------------------CAMBIO Y DELTA------------------------------------
	/*
	fprintf(screen, "First atom coordinates Calpha: x: %.2f, y: %.2f, z: %.2f\n",xca[0][0],xca[0][1],xca[0][2]);
	fprintf(screen, "First atom coordinates Cbeta: x: %.2f, y: %.2f, z: %.2f\n",xcb[0][0],xcb[0][1],xcb[0][2]);
	fprintf(screen, "First atom coordinates O: x: %.2f, y: %.2f, z: %.2f\n",xo[0][0],xo[0][1],xo[0][2]);
	fprintf(screen, "First atom coordinates N: x: %.2f, y: %.2f, z: %.2f\n",xn[0][0],xn[0][1],xn[0][2]);
	fprintf(screen, "First atom coordinates C': x: %.2f, y: %.2f, z: %.2f\n",xcp[0][0],xcp[0][1],xcp[0][2]);
	fprintf(screen, "First atom coordinates H: x: %.2f, y: %.2f, z: %.2f\n",xh[0][0],xh[0][1],xh[0][2]);
	fprintf(screen,"~~~~~~~\n");
	fprintf(screen, "Last atom coordinates Calpha: x: %.2f, y: %.2f, z: %.2f\n",xca[n-1][0],xca[n-1][1],xca[n-1][2]);
	fprintf(screen, "Last atom coordinates Cbeta: x: %.2f, y: %.2f, z: %.2f\n",xcb[n-1][0],xcb[n-1][1],xcb[n-1][2]);
	fprintf(screen, "Last atom coordinates O: x: %.2f, y: %.2f, z: %.2f\n",xo[n-1][0],xo[n-1][1],xo[n-1][2]);
	fprintf(screen, "Last atom coordinates N: x: %.2f, y: %.2f, z: %.2f\n",xn[n-1][0],xn[n-1][1],xn[n-1][2]);
	fprintf(screen, "Last atom coordinates C': x: %.2f, y: %.2f, z: %.2f\n",xcp[n-1][0],xcp[n-1][1],xcp[n-1][2]);
	fprintf(screen, "Last atom coordinates H: x: %.2f, y: %.2f, z: %.2f\n",xh[n-1][0],xh[n-1][1],xh[n-1][2]);
	*/
	...
	//fprintf(screen,"rnd_idx: %d, place_change:%d \n",rnd_idx,place_change);
	...
	// Compute of the electrostatic interactions of the residue in place_change 
     if (i != rnd_idx)
     {
		if(place_change<n && charged_indexes[i]<n){
			...
			old_energy+=electro_i_j_terminal(place_change, charged_indexes[i], charge_on_residues);
		}

     }
	//fprintf(screen,"Partial Old energy: %8.6f\n",old_energy);
    }
	...
	for(int i=0;i<total_res_charged;i++){
     if (i != rnd_idx)
     {
	   if(place_change<n && charged_indexes[i]<n){
			...
			new_energy+=electro_i_j_terminal(place_change, charged_indexes[i], charge_on_residues);
	   }
...
/*---------------------------------------------WRSH------------------------------------------*/
	...
	if(place_change<n){
		resno = res_no[place_change]-1;
		res_type = se_map[se[resno]-'A'];
		let = one_letter_code[res_type];
		fprintf(screen,"Place change letter:%c , Number: %d\n",let,place_change+1);
		fprintf(screen,"Polar\n");
		Npol=count_pol(place_change);
		fprintf(screen,"Non Polar\n");
		Nnonpol=count_nonpol(place_change);

//----------------------------------------------------------------------------
		//Momentaneo asignacion de los parametros A_self_pol y A_self_nonpol
		double A_self_pol;
		double A_self_nonpol;
		if(let=='D'){
			A_self_pol = -0.5;
			A_self_nonpol = 2.5;
		}else if(let=='E'){
...
int FixBackbone::charge_flip(int charge, char letter){
	int charge_flipped;
	if(letter=='D' || letter=='E'|| letter=='C'){ // Acid
		if(charge==-1){
			charge_flipped=0;
		  }else if(charge==0){
			charge_flipped=-1;
	...
	}else if(letter=='R' || letter=='K' || letter=='H' || letter=='N'){ // Base
		if(charge==1){
			charge_flipped=0;
		  }else if(charge==0){
			charge_flipped=1;
...
//Calculo de la energia electrostatica entre i y j si ninguno es terminal
	  //fprintf(screen,"electroij\n");
...
double FixBackbone::electro_i_j_terminal(int i, int j, double *array){ 
	//fprintf(screen,"electroijterminal\n");
	//fprintf(screen,"i: %d, j: %d \n",i,j);
//Calculo de la energia electrostatica entre i y j si alguno o los dos son terminal
	...
	//Testeo de separacion con virtual i,j
	if (abs(virtuali-virtualj)<debye_huckel_min_sep) return 0.0; //Distancia en residuos minima necesaria
	    
	charge_i = array[i];  // La carga se asigna bien porque el vector mide 2 demas
	charge_j = array[j]; 

	if (charge_i == 0 && charge_j == 0) return 0.0; // Esto despues no va a hacer falta
											  // Voy a hacer la cuenta solo con los cargados directamente

	//Veo si los dos son terminal
	if((i==n && j==n+1)||(i==n+1 && j==n)){
		//Ambos son terminal
		xi=xca[0];
		xj=xo[n-1];

	}else{//Alguno no es terminal, va a ser el i. Defino el xj
		if(i==n){
			xj=xca[0];
			i=j;
		}else if(i==n+1){
			...
			xj=xo[n-1];
			i=i;
		}//El i que sale de aca es un residuo
		int i_resno = res_no[i]-1;
		int ires_type = se_map[se[i_resno]-'A'];
		if (se[i_resno]=='G') { xi = xca[i]; iatom = alpha_carbons[i]; } // Si es una G la posicion viene dada por el alpha
		else { xi = xcb[i]; iatom  = beta_atoms[i]; }                    // Si no viene dada por el beta. Para el i y para el j
...
double FixBackbone::count_pol(int i){
	double Ni=0; //Without considering terminals
	double dx[3];  
	double *xi, *xk, r;
	int iatom;
	...
	for(k=0;k<n;k++){
		...
		if(let=='C'||let=='D'||let=='E'||let=='H'||let=='K'||let=='N'||let=='Q'||let=='R'||let=='S'||let=='T'||let=='Y'){
			...
			dx[2] = xi[2] - xk[2];
			r = sqrt(dx[0]*dx[0]+dx[1]*dx[1]+dx[2]*dx[2]); 
			if(r<rpol){ //rpol & alpha_pol defined outside
				Ni=Ni+1;
				fprintf(screen, "Neighbour inside: %c\n",let);
			}else{
				Ni=Ni+exp(-alpha_pol*((r-rpol)*(r-rpol)));
			}
		}
...
double FixBackbone::count_nonpol(int i){
	double Ni=0; //Without considering terminals
	double dx[3];  
	double *xi, *xk, r;
	int iatom;
	...
	for(k=0;k<n;k++){
		...
		kres_type = se_map[se[k_resno]-'A'];
		let = one_letter_code[kres_type];
		if(let=='A'||let=='V'||let=='I'||let=='L'||let=='M'||let=='F'||let=='W'||let=='P'){
			xk = xcb[k];
			dx[0] = xi[0] - xk[0];   
			dx[1] = xi[1] - xk[1];
			dx[2] = xi[2] - xk[2];
			r = sqrt(dx[0]*dx[0]+dx[1]*dx[1]+dx[2]*dx[2]); 
			if(r<rnonpol){ //rnonpol & alpha_pol defined outside
				Ni=Ni+1;
				fprintf(screen, "Neighbour inside: %c\n",let);
			}else{
				Ni=Ni+exp(-alpha_nonpol*((r-rpol)*(r-rpol)));
			}
		}
...
double FixBackbone::energy_self(int i, int place_change, int charge, double A_self_pol, double A_self_nonpol){
	double Npol,Nnonpol;	
	int resno,res_type;
	char let;
	double e_self=0;
	...
	//Polars
	if(Npol>NpolMax){
		e_self=e_self+A_self_pol;
	}else{
		e_self=e_self+A_self_pol*exp(-alpha_u_pol*(Npol-NpolMax)*(Npol-NpolMax));
	}
	//Non Polars
	if(Nnonpol>NnonpolMax){
		e_self=e_self+A_self_nonpol;
	}else{
		e_self=e_self+A_self_nonpol*exp(-alpha_u_nonpol*(Nnonpol-NnonpolMax)*(Nnonpol-NnonpolMax));
	}
	return e_self;	
}
//----- AGREGADO FRANCO FIN -----
